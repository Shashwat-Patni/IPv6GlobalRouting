...
if multiple exit Interfaces are present from the root node to a network, then Packets may travel an extra hop to reach their destination in that network.
...
do one unnecessary hop, passing though the designated router, and discarding the (shorter) path
