...
void
SPFVertex::SetNode(Ptr<Node> node)
{
...
GlobalRouteManagerImpl::SPFAddASExternal(GlobalRoutingLSA* extlsa, SPFVertex* v)
{
    ...
    if (!node)
    {
        NS_LOG_ERROR("SPFAddASExternal():Can't find root node " << routerId);
        return;
    }
    //
    ...
    NS_LOG_ERROR("SPFIntraAddRouter(): routerId and vertex ID do not match");
}
...
GlobalRouteManagerImpl::SPFIntraAddTransit(SPFVertex* v)
{
    ...
    // This should never happen. The RouterId and vertex Id should match
    NS_LOG_ERROR("Root Node not found to Add Routes for in SPFIntrAddTransit()");
}
